...
public abstract class Chunk(Chunk? parent, ChunkHeader header) : IBinaryReadWrite, IChunkData
{
    ...
    public static Chunk? ReadChunk(BinaryReader fileAccess)
    {
        ...
        // Print debug message
        Console.WriteLine(
            $"Chunk.ReadChunk: Reading chunk type: {header.Type} at position {fileAccess.BaseStream.Position} with size {header.Size:X}");
        chunk.Read(fileAccess);

        // Print debug message
        Console.WriteLine(
            $"Chunk.ReadChunk: Read chunk type: {header.Type} up to position {fileAccess.BaseStream.Position} with size {header.Size:X}");
        // Print debug message
        ...
        if (fileAccess.BaseStream.Position != chunk.EndPosition + 12)
            // Print debug message
            Console.WriteLine(
                $"Chunk.ReadChunk: Something went wrong, we should be at position {chunk.EndPosition + 12} (Chunk End) but we are at {fileAccess.BaseStream.Position}");
    ...
    public virtual ChunkNode ToChunkNode()
    {
        ...
        {
            Name = GetChunkDisplayName(),
            Type = GetType().Name
        };

        // Add basic chunk properties
        AddBasicChunkProperties(node);

        // Add chunk-specific properties
        AddChunkSpecificProperties(node);

        // Add children if this chunk has any
        AddChildChunks(node);

        return node;
    }

    protected virtual string GetChunkDisplayName()
    {
        ...
        return Type switch
        {
            ...
            _ => GetType().Name.Replace("Chunk", "")
        };
    }

    protected virtual void AddBasicChunkProperties(ChunkNode node)
    {
        node.AddProperty("ChunkType", Type.ToString());
        node.AddProperty("HeaderSize", Header.Size);
    }

    protected virtual void AddChunkSpecificProperties(ChunkNode node)
    {
        // Override in derived classes to add specific properties
    }

    protected virtual void AddChildChunks(ChunkNode node)
    {
        // Override in derived classes that have child chunks
        // Most chunks don't have direct child chunks in the base implementation
    }
}
...
public class ChunkHeader
{
    ...
    {
        Console.WriteLine("ChunkHeader.ReadHeader: Reading chunk header at position: '" +
                          binaryReader.BaseStream.Position + "'");
        var header = new ChunkHeader();
        header.Read(binaryReader);
        Console.WriteLine(
            $"ChunkHeader.ReadHeader: Read chunk header type: '{header.Type}' size: '{header.Size:X}' version: '{LibraryIdUtils.GetVersionString(header.Version)}'");
        return header;
    }

    public static void WriteHeader(BinaryWriter binaryWriter, ChunkHeader chunkHeader)
    {
        Console.WriteLine("ChunkHeader.WriteHeader: Writing chunk header at position: '" +
                          binaryWriter.BaseStream.Position + "'");
        chunkHeader.Write(binaryWriter);
        Console.WriteLine(
            $"ChunkHeader.WriteHeader: Wrote chunk header type: '{chunkHeader.Type}' size: '{chunkHeader.Size:X}' version: '{LibraryIdUtils.GetVersionString(chunkHeader.Version)}'");
    }
}
...
public class NodeNameChunk(ExtensionChunk? parent, ChunkHeader header) : Chunk(parent, header)
{
    ...
    {
        Console.WriteLine(
            $"NodeNameChunk.Read: Reading node name chunk at position: '{binaryReader.BaseStream.Position}'");
        ...
        Console.WriteLine(
            $"NodeNameChunk.Read: Read node name chunk up to position: '{binaryReader.BaseStream.Position}'");
    }
}
...
public class ExtensionChunk : Chunk
{
    ...
    public override void Read(BinaryReader binaryReader)
    {
        // Print debug message
        Console.WriteLine(
            $"ExtensionChunk.Read: Reading extension chunk at position: '{binaryReader.BaseStream.Position}'");
        ...
        // Print debug message
        Console.WriteLine(
            $"ExtensionChunk.Read: Read extension chunk up to position: '{binaryReader.BaseStream.Position}'");
    }
    ...
    protected override void AddChildChunks(ChunkNode node)
    {
        ...
        if (Chunks != null)
        {
            foreach (var chunk in Chunks)
            {
                if (chunk != null)
                {
                    var childNode = chunk.ToChunkNode();

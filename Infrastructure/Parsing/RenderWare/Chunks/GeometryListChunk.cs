...
{
    public List<GeometryChunk> Geometries = null!;
    private GeometryListStructChunk geometryListStruct = null!;
    ...
    {
        // Add geometry list struct
        if (geometryListStruct != null)
        {
            var geometryListStructNode = geometryListStruct.ToChunkNode();
            node.AddChild(geometryListStructNode);
        }
        ...
        if (Geometries != null)
        {
            foreach (var geometry in Geometries)
            {
                var geometryNode = geometry.ToChunkNode();

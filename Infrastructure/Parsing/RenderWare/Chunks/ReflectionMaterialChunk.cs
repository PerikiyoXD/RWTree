...
*/
    ...
    {
        Console.WriteLine(
            $"ReflectionMaterialChunk.Read: Reading reflection material chunk at position: '{binaryReader.BaseStream.Position}'");
        ...
        if (TexturePtr != 0)
            Console.WriteLine(
                $"ReflectionMaterialChunk.Read: Expected texture pointer to be 0, but got 0x{TexturePtr:X8} instead");

        Console.WriteLine(
            $"ReflectionMaterialChunk.Read: Read reflection material chunk up to position: '{binaryReader.BaseStream.Position}'");
    }
}